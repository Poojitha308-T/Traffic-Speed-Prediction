...
def load_data():
    try:
        ...
        # Historical statistics per segment
        ...
        # Segment-specific speed by time of day and weekday
...
def main():
    ...
    'is_rush_hour', 'is_midday', 'log_segment_length', 'segment_length_squared',
        ...
        try:
            ...
            logger.error(f"Failed to save selected_features.pkl: {e}")
            raise
        ...
        for model, name in models:
            ...
            try:
                ...
                logger.error(f"Failed to save {name.replace(' ', '_')}_model.pkl: {e}")
                raise
        ...
        required_files = ['scaler.pkl', 'selected_features.pkl', 'kmeans_model.pkl']
        missing_files = [f for f in required_files if not os.path.exists(f)]
        if missing_files:
            logger.error(f"Required files not created: {missing_files}")
            raise FileNotFoundError(f"Required files not created: {missing_files}")
        logger.info(f"All required files created: {os.listdir('.')}")
